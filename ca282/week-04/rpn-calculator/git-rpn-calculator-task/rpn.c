/*
 * This is an RPN calculator written in C (source: https://gist.github.com/jandk/1709771).
 *
 * Unfortunately, it contains some artificially-introduced errors.
 */
...
int eval( char op, stack** head )
{
	...
	switch( op ) {
		...
		case '*': return pop(head) * pop(head); /* FIXME! */
...
int main( int argc, char** argv )
{
	...
	for( i = 1; i < argc; i++ ) {
		...
		if( checknr( token ) ) {
			...
			op = token[0]; /* And something is wrong here. */
